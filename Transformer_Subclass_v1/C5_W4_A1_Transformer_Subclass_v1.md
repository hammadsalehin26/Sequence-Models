...
def scaled_dot_product_attention(q, k, v, mask):
    ...
    if mask is not None:
        ...
        scaled_attention_logits += (1.0 - tf.cast(mask, tf.float32)) * -1e9
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask):
        ...
        # START CODE HERE
        # Self-attention
        self_mha_output = self.mha(query=x, value=x, key=x, attention_mask=mask, training=training)  # (batch_size, input_seq_len, embedding_dim)
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, look_ahead_mask, padding_mask):
        ...
        # BLOCK 2: Multi-head attention with encoder output
        mult_attn_out2, attn_weights_block2 = self.mha2(
            query=Q1, value=enc_output, key=enc_output, attention_mask=padding_mask, 
            return_attention_scores=True, training=training
        )
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, 
        ...
        for i in range(self.num_layers):
            # pass x and the encoder output through a decoder layer and save the attention weights
            x, block1, block2 = self.dec_layers[i](x, enc_output, training, look_ahead_mask, padding_mask)

            # update attention_weights dictionary
...
class Transformer(tf.keras.Model):
    ...
    def call(self, input_sentence, output_sentence, training, enc_padding_mask, look_ahead_mask, dec_padding_mask):
        ...
        Returns:
            final_output -- Tensor of shape (batch_size, tar_seq_len, target_vocab_size)
